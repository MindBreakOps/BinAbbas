...
export default function FinancialExpenses() {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], amount: '', category: 'رواتب وأجور', description: '' });

  const fetchExpenses = async () => {
	setIsLoading(true);
	const { data } = await supabase.from('financials').select('*').eq('type', 'مصروف').order('date', { ascending: false });
  ...
  const openCreateModal = () => {
	setEditingId(null);
	setFormData({ date: new Date().toISOString().split('T')[0], amount: '', category: 'رواتب وأجور', description: '' });
	setIsModalOpen(true);
  };
  ...
  const openEditModal = (item: any) => {
	...
	setFormData({
	  ...
	  category: item.category || 'رواتب وأجور', description: item.description || ''
	});
	setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
	e.preventDefault();
	const payload = { type: 'مصروف', date: formData.date, amount: parseFloat(formData.amount), category: formData.category, description: formData.description };
  ...
  const handleDelete = async (id: string) => {
	if (!window.confirm('تأكيد حذف هذا المصروف؟')) return;
  ...
  const styles: { [key: string]: React.CSSProperties } = {
	...
	btnPrimary: { backgroundColor: '#dc2626', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '4px', fontWeight: 700, cursor: 'pointer', display: 'flex', gap: '8px', alignItems: 'center' },
  ...
  return (
	<div>
	  <div style={styles.header}>
		<div><h2 style={styles.title}>سجل المصروفات والنفقات</h2><p style={styles.subtitle}>تسجيل وتتبع التدفقات النقدية الخارجة من الصندوق</p></div>
		<button style={styles.btnPrimary} onClick={openCreateModal}><Icon name="plus" size={16} /> سند صرف جديد</button>
	  </div>
	  {isLoading ? <p>جاري التحميل...</p> : (
		<table style={styles.table}>
		  ...
		  <tbody>
			{expenses.map(e => (
			  <tr key={e.id}>
				<td style={{...styles.td, color: '#6b7280'}}>{e.date}</td>
				<td style={styles.td}>{e.category}</td>
				<td style={styles.td}>{e.description || '—'}</td>
				<td style={{...styles.td, color: '#dc2626', fontWeight: 900}}>{Number(e.amount).toLocaleString()}</td>
				<td style={styles.td}>
				  <div style={{display: 'flex', gap: '12px'}}>
					<button style={styles.btnEdit} onClick={() => openEditModal(e)}>تعديل</button>
					<button style={styles.btnDelete} onClick={() => handleDelete(e.id)}>حذف</button>
				  </div>
				</td>
			  </tr>
			))}
			{expenses.length === 0 && <tr><td colSpan={5} style={{textAlign: 'center', padding: '24px', color: '#6b7280'}}>لا توجد سجلات.</td></tr>}
		  </tbody>
		</table>
	  )}
	  ...
	  {isModalOpen && (
		<div style={styles.modalOverlay}>
		  <div style={styles.modalContent}>
			<h3 style={{ margin: '0 0 24px 0', fontWeight: 800 }}>{editingId ? 'تعديل السند' : 'إصدار سند صرف (مصروف)'}</h3>
			<form onSubmit={handleSave}>
			  ...
			  <div style={styles.inputGroup}>
				...
				<select style={styles.input} value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
				  <option>رواتب وأجور</option><option>مصاريف تشغيلية</option><option>مكافآت وجوائز</option><option>صيانة</option><option>أخرى</option>
				</select>
			  </div>
			  <div style={styles.inputGroup}>
